...
// A gap is a removal from the middle, which contiguity does catch.
func TestGapInEmbeddedSetIsRefused(t *testing.T) {
	// loadMigrations enforces this over the real directory; the message is what
	// an operator sees, so assert it says renumbering is not the fix.
	got, err := loadMigrations()
	if err != nil {
		t.Fatalf("loadMigrations: %v", err)
	}
