...
- ~~**Delete the bootstrap escape in `make breaking`.**~~ *Discharged 2026-08-17.* Until this
  record's own change was on the main branch there was no previous schema to compare against, so the
  target said so and exited — correct exactly once. A base ref without a schema is now a hard
  failure, and "both are enforced by the build" is true without qualification.
