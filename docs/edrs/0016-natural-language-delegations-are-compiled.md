...
- **2026-08-19**: Amended for the artefact spelling ([EDR-0041](./0041-one-spelling-for-a-scope.md)): the compiled delegation's `fence` is an array of conjuncts, its relation is two fields, and its operations are lowercase. The `derivation` entries stay prose — they are an account for a human of how a clause was compiled, not a copy of the compiled field. The cost lands here rather than anywhere else: the grantor now signs a list where they used to sign a predicate.
- **2026-08-19**: Amended a second time: the compiler's output constraints required an emitted fence to "parse under EDR-0007's grammar", which is a *statement* grammar — the same false attribution removed from EDR-0007 in this change, in the one other place it appeared. They are now stated per conjunct, against [EDR-0041](./0041-one-spelling-for-a-scope.md)'s shape rules and EDR-0007 rule 5's target-relation requirement, with what a conjunct may *do* left open as [issue #25](https://github.com/sixfathoms/marque/issues/25).
