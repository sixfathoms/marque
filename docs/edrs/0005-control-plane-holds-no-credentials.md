...
- **2026-08-20**: Amended a third time, same day. The mechanism sentence said "a test that parses every first-party file"; EDR-0042's mechanism is now a `go list -deps` graph check over each binary, with that walk demoted to a cross-check. The line below describes the previous shape and is left standing for the sequence. The decision is untouched.
