...
- **2026-08-19**: Amended with a pointer: a fence is a list of conjuncts ([EDR-0041](./0041-one-spelling-for-a-scope.md)), so the canonical `display` renders a list rather than a single predicate. The worked example above predates that and still shows one predicate; what the list rendering is — ordering, delimiter, escaping, truncation — is undefined and is [issue #23](https://github.com/sixfathoms/marque/issues/23), due before M3. The rendering rules are versioned and the version is in the payload, which is the mechanism that absorbs a change like this; the decision is unchanged.
