...
- **M0's exit criterion reads against those tiers** rather than against a phrase that meant whatever
  the reader assumed, and M1's integration test says where it runs. **M2's now carries the obligation
  to widen the test tier**, since M2 is where the parser becomes C and the person implementing it
  will read M2's criterion rather than a table three sections earlier.
