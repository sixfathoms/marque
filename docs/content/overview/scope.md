...
| **Threshold approvals *within a single stage*** (`min_approvals > 1`) | Multi-stage escalation chains ship in the first release and already place several approver signatures on one marque ([EDR-0030](../../edrs/0030-a-marque-states-its-own-approval-requirement.md), [EDR-0019](../../edrs/0019-escalation-is-a-chain.md)) — what is deferred is requiring *two people at the same stage*. The payload encodes it (`stages[].required`); the review UX for collecting a second signature at one stage does not exist. A policy-driven second **stage** does apply to human submitters in the first release |
| Emergency access with the control plane unreachable | Needs a pre-issued, longer-lived marque shape and a careful ceremony |
